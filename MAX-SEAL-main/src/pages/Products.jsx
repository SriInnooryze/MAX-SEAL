...
export default function Products() {
  ...
  // "Browse all products" is a real navigation to the separate Explore All
  // Products page (routes.exploreProducts), not an internal phase — see the
  // <Link>s below rather than a state-setting handler here.
    ...
    <main>
      <PageHero
          ...
          <div className="phero__actions">
            ...
            <Link className="ms-btn ms-btn--on-dark ms-btn--lg" to={routes.exploreProducts}>View All Products</Link>
          </div>
        </PageHero>
          ...
          <div className="wrap">
            ...
            {phase === 'choose' && (
              <div className="paths" key="choose">
                ...
                <Link className="path" to={routes.exploreProducts}>
            ...
            {phase === 'q1' && (
              <div className="jpanel" key="q1">
                ...
                <Link className="jnav__skip" to={routes.exploreProducts}>Skip and view all products</Link>
              </div>
            )}
                    ...
                    {results.map(r => {
                      ...
                      return (
                        <div key={r.id} className={'rrow' + (on ? ' cmp' : '')}>
                          <div className="rrow__thumb"><image-slot id={'rrow-' + r.id} src={r.image} shape="rect" fit="contain" placeholder={r.code} /></div>
            ...
            <table className="cmptable">
              <thead>
                <tr>
                  ...
                  {compare.map(id => { const f = FAMILIES.find(x => x.id === id); return (
                    <th key={id} scope="col">
                      <span className="cmptable__thumb"><image-slot id={'cmp-h-' + id} src={f.image} shape="rect" fit="contain" placeholder={f.code} /></span>
                      {f.name}
                    </th>
                  ); })}
                </tr>
              ...
              <tbody>
                ...
                <tr><th scope="row">Material or Lining Options</th>{compare.map(id => { const m = FAMILIES.find(x => x.id === id).materials.seatLiningOptions; return <td key={id}>{m && m !== 'To be validated' ? m : <span className="tbv">To be confirmed</span>}</td>; })}</tr>
